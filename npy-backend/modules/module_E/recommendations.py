...
# ═══════════════════════════════════════════════════════════════════════════
# 2. CITATIONS TRACKER RECOMMENDATIONS
# ═══════════════════════════════════════════════════════════════════════════

def generate_citations_recommendations(
    ranking_analysis: Dict[str, Any],
) -> Dict[str, Any]:
    """
    Analyse citation-level fields in ranking_analysis and generate
    improvement recommendations for the Citations Tracker section.

    Inputs used:
      - ranking_position_per_prompt  (position, percentile, source_diversity,
        credibility_score, accuracy_score, sentiment_score, citation_matched)
    ...
    rows: List[Dict[str, Any]] = ranking_analysis.get("ranking_position_per_prompt", [])
    model_wise: List[Dict] = ranking_analysis.get("model_wise_comparison", [])
    ...
    positions   = [r["position"] for r in cited_rows if r.get("position") is not None]
    percentiles = [r["percentile"] for r in cited_rows if r.get("percentile") is not None]
    diversity_scores = [r.get("source_diversity", 0) for r in rows]
    credibility_scores = [r.get("credibility_score", 0) for r in cited_rows]
    accuracy_scores  = [r.get("accuracy_score", 0.0) for r in rows if r.get("accuracy_score") is not None]
    sentiment_scores = [r.get("sentiment_score", 0.0) for r in rows if r.get("sentiment_score") is not None]
    ...
    # Determine how many unique models are citing the brand
    models_with_citations: set = set()
    for row in cited_rows:
        m = row.get("model")
        if m:
            models_with_citations.add(m)

    total_unique_models = len(set(r.get("model") for r in rows if r.get("model")))

    # ── Rule 1: No citations at all ──────────────────────────────────────────
    if len(cited_rows) == 0:
        recs.append(_rec(
            1, "Citations Tracker", SEVERITY_CRITICAL,
            "Brand URL has zero AI citations",
            "No AI model (ChatGPT or Gemini) cited the brand URL in any tracked prompt response. "
            "The brand is invisible to AI-driven discovery.",
            "Ensure your pages are crawlable and indexed by Google. Build external backlinks "
            "from authoritative domains. Publish long-form, question-answering content that "
            "matches the exact tracked prompts using structured data (FAQ schema).",
            "Brand URL begins appearing as a cited source in AI model responses.",
            ["ranking_analysis.ranking_position_per_prompt"],
        ))
        return {"recommendations": recs, "health_score": 0, "summary": "Brand URL is not cited by any AI model."}

    # ── Rule 2: High average position (cited late in the list) ───────────────
    if avg_position is not None and avg_position > 5:
        recs.append(_rec(
            1, "Citations Tracker", SEVERITY_WARNING,
            f"Brand cited late in AI responses (avg position #{avg_position:.0f})",
            "When the brand IS cited, it appears deep in the AI citation list, meaning it is "
            "not considered a primary or authoritative source for the topic.",
            "Increase topical authority by creating comprehensive pillar content and "
            "topic clusters. Earn backlinks from high-authority domains in your industry. "
            "Ensure your page is the single most comprehensive resource on the topic.",
            "Brand moves to top 3 citation positions, improving referral clicks from AI responses.",
            ["ranking_analysis.ranking_position_per_prompt"],
        ))
    elif avg_position is not None and avg_position > 3:
        recs.append(_rec(
            2, "Citations Tracker", SEVERITY_INFO,
            f"Brand cited at mid-level positions (avg #{avg_position:.0f})",
            "The brand is cited but not in the top 3 positions. There is room to strengthen "
            "topical relevance.",
            "Update and expand existing pages to improve depth, add real-world examples, "
            "case studies, and structured data to boost perceived authority.",
            "Improves average citation position toward top-3, increasing AI-driven referral traffic.",
            ["ranking_analysis.ranking_position_per_prompt"],
        ))

    # ── Rule 3: Low average percentile ───────────────────────────────────────
    if avg_percentile is not None and avg_percentile < 40:
        recs.append(_rec(
            2, "Citations Tracker", SEVERITY_WARNING,
            f"Low citation percentile ({avg_percentile:.0f}th percentile)",
            "The brand is cited, but ranks in the lower half of AI responses. Other sources "
            "are being preferred over the brand.",
            "Analyse which competing sources AI models prefer and identify content gaps. "
            "Create content that directly addresses the same topics with more depth, "
            "better structure, and stronger E-E-A-T signals (expertise, experience, authoritativeness, trust).",
            "Raises percentile ranking, pushing the brand to the top of AI citation lists.",
            ["ranking_analysis.percentile_by_prompt"],
        ))

    # ── Rule 4: Brand cited by only one model ────────────────────────────────
    if len(models_with_citations) == 1 and total_unique_models > 1:
        model_name = list(models_with_citations)[0]
        recs.append(_rec(
            2, "Citations Tracker", SEVERITY_WARNING,
            f"Only one AI model cites the brand ({model_name})",
            f"The brand URL is only cited by {model_name}. Other AI models do not include "
            "the brand in their responses, creating uneven AI search visibility.",
            "Diversify your SEO signals: submit to Bing Webmaster Tools (used by some AI "
            "models), create a Wikipedia or Wikidata entry if eligible, maintain consistent "
            "brand information across Google Business Profile, LinkedIn, and Crunchbase.",
            "Brand citations spread across multiple AI platforms, increasing total AI reach.",
            ["ranking_analysis.model_wise_comparison"],
        ))

    # ── Rule 5: Low source diversity ─────────────────────────────────────────
    if avg_diversity < 40:
        recs.append(_rec(
            2, "Citations Tracker", SEVERITY_WARNING,
            f"Low source diversity in AI responses ({avg_diversity:.0f}/100)",
            "AI models are pulling citations from a very narrow set of domains in these "
            "responses. This indicates the topic is not well-covered across the web, "
            "making it harder for your brand to earn citations.",
            "Pursue guest posting and content partnerships with diverse industry blogs, "
            "news sites, and directories. The more varied the ecosystem of sources "
            "mentioning your brand, the better.",
            "Increases source diversity, making AI-cited content more balanced and trustworthy.",
            ["ranking_analysis.ranking_position_per_prompt"],
        ))

    # ── Rule 6: Low accuracy score ────────────────────────────────────────────
    if avg_accuracy < 60 and avg_accuracy > 0:
        recs.append(_rec(
            2, "Citations Tracker", SEVERITY_WARNING,
            f"AI descriptions of brand are inaccurate (accuracy {avg_accuracy:.0f}/100)",
            "When AI models generate responses that cite the brand, the descriptions of the "
            "brand's offerings are not sufficiently accurate. This can mislead potential customers.",
            "Add a clear, factual 'About' page and a Services/Products page with precise, "
            "jargon-free language. Use structured data (Organization schema) to define your brand. "
            "Publish press releases to establish authoritative information online.",
            "AI models produce more accurate brand descriptions, reducing misinformation and "
            "improving user trust.",
            ["ranking_analysis.ranking_position_per_prompt"],
        ))

    # ── Rule 7: Negative / low sentiment in AI citations ─────────────────────
    if avg_sentiment < 0.4 and len(cited_rows) > 0:
        recs.append(_rec(
            2, "Citations Tracker", SEVERITY_WARNING,
            f"Low sentiment in AI citation context (score {avg_sentiment:.2f})",
            "The context in which the brand is cited by AI models carries a neutral or negative "
            "sentiment. This could affect user perception when they see the brand in AI answers.",
            "Address negative reviews and PR issues online. Publish success stories, case "
            "studies, and customer testimonials. Encourage positive coverage from industry "
            "analysts and review platforms.",
            "Improves the sentiment of AI-generated brand mentions, leading to better user impressions.",
            ["ranking_analysis.ranking_position_per_prompt"],
        ))

    # ── Rule 8: URL credibility ───────────────────────────────────────────────
    if avg_credibility < 50 and cited_rows:
        recs.append(_rec(
            3, "Citations Tracker", SEVERITY_INFO,
            "Low credibility score for citation sources",
            "The AI responses that cite the brand draw from low-credibility source domains. "
            "This can reduce how prominently your content is featured.",
            "Build backlinks from high-authority, credible domains (.edu, .gov, established "
            "news sites). Aim to get the brand mentioned in industry reports and whitepapers.",
            "Higher credibility sources citing your brand improves overall AI citation quality.",
            ["ranking_analysis.ranking_position_per_prompt"],
        ))
    ...
    # Summary
    if not recs:
        summary = "Citations are performing well. Maintain authority-building to sustain top positions."
    elif avg_position and avg_position <= 3:
        summary = f"Brand appears at position #{avg_position:.0f} on average. Focus on cross-model citation coverage."
    else:
        summary = f"Brand is cited but needs position and accuracy improvements. Avg position: #{avg_position:.0f}." if avg_position else "Citations need improvement."
...
# ═══════════════════════════════════════════════════════════════════════════
# 3. SHARE OF VOICE RECOMMENDATIONS
# ═══════════════════════════════════════════════════════════════════════════
