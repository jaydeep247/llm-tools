...
def _estimate_sentiment(text: str, brand_name: str) -> float:
    ...
    Uses keyword heuristics on a ±200 char context window.
    """
    idx = text.lower().find(brand_name.lower())
    if idx == -1:
        return 0.0
    ctx = text[max(0, idx - 200): min(len(text), idx + 200)].lower()

    positive = [
        "best", "top", "leading", "excellent", "great", "innovative",
        "popular", "recommended", "powerful", "comprehensive", "trusted",
        "award", "winner", "superior", "advanced", "reliable", "fast",
    ]
    negative = [
        "slow", "expensive", "limited", "complex", "difficult", "bad",
        "poor", "weak", "overpriced", "buggy", "outdated", "deprecated",
    ]
...
def _aggregate_benchmark_scores(
    ...
    for entity_name, row in aggregate.items():
        ...
        # We don't have per-snapshot citation_scores here (those live in
        # cbm_citation_snapshots). At the visibility-comparison stage we compute
        # a proxy using the two most informative signals from that table:
        #   • visibility_score  = % of models that mentioned the entity (0-100)
        #   • avg_rank_percentile = how early the entity appeared when mentioned
        # Weight: 60% presence × 40% position — matches SOP signal weights
        # (presence signals = URL 30 + mention 25 = 55% of 100; position = 20%)
        row["benchmark_score"] = round(
            (row["visibility_score"] * 0.6) + (row["avg_rank_percentile"] * 0.4), 2
        )
            ...
            try:
                ...
                for probe_model in self.models:
                    ...
                    if present:
                        ...
                        # Recompute benchmark for brand after probe
                        aggregate[brand_key]["benchmark_score"] = round(
                            (aggregate[brand_key]["visibility_score"] * 0.6) + (rp * 0.4), 2
                        )
                        break
            except Exception:
                pass
