"""
module_d_runner.py — updated to include recommendation generation
Add run_recommendations() and update run_module_d() to call it.

Paste this into your existing module_d runner file (replace the
run_module_d function and add run_recommendations below it).
"""
...
async def run_recommendations(
    ...
    """
    Generate IEU-ranked recommendation cards from existing prompt metrics.
    Call this AFTER run_prompt_tracking() has produced prompt_metrics.
    """
...
async def run_module_d(
    ...
    """
    Full Module D analysis — now includes recommendation engine output.
    """
    if not html_content:
    ...
    # Run entity analysis + content metrics in parallel
    ...
    result = {
        ...
        # NEW field — recommendation cards ready for frontend
    ...
    try:
        ...
        doc = {"jobId": job_id, "url": url, "createdAt": datetime.utcnow(), **result}
...
async def run_prompt_tracking(job_id, url, prompts):
    mongo_manager.connect()
    ai_service = ClaudeService()
...
async def run_content_metrics(job_id, url, html_content=None):
    if not html_content:
        html_content = load_raw_html_sync(job_id)
    if not html_content:
        return {"error": "HTML content missing"}
...
async def run_entity_analysis(job_id, url, html_content=None):
    if not html_content:
        html_content = load_raw_html_sync(job_id)
    if not html_content:
        return {"error": "HTML content missing"}
