...
import { HealthChecker } from '../../controllers/module_D/monitoring/HealthCheck.js';
  ...
  try {
    ...
    res.json({
      pages: pages.map((p: any) => ({
        ...
        tableCount: p.tableCount ?? p.table_count ?? null,
        tableData: p.tableData ?? p.table_data ?? null,
        hasTables: p.hasTables ?? p.has_tables ?? null,
        faqCount: p.faqCount ?? p.faq_count ?? null,
        faqData: p.faqData ?? p.faq_data ?? null,
        hasFaqs: p.hasFaqs ?? p.has_faqs ?? null,
        faqScore: p.faqScore ?? p.faq_score ?? null,
        faqDetectionMethod: p.faqDetectionMethod ?? p.faq_detection_method ?? null,
        faqSchemaPresent: p.faqSchemaPresent ?? p.faq_schema_present ?? null,
        hasMixedContent: p.hasMixedContent ?? p.has_mixed_content ?? null,
        mixedContentSeverity: p.mixedContentSeverity ?? p.mixed_content_severity ?? null,
        mixedContentData: p.mixedContentData ?? p.mixed_content_data ?? null,
        activeMixedContentCount: p.activeMixedContentCount ?? p.active_mixed_content_count ?? null,
        passiveMixedContentCount: p.passiveMixedContentCount ?? p.passive_mixed_content_count ?? null,
        totalInsecureResources: p.totalInsecureResources ?? p.total_insecure_resources ?? null,
        headerStructureData: p.headerStructureData ?? p.header_structure_data ?? null,
        headerStructureIssues: p.headerStructureIssues ?? p.header_structure_issues ?? null,
        viewportPresent: p.viewportPresent ?? p.viewport_present ?? null,
        viewportContent: p.viewportContent ?? p.viewport_content ?? null,
        viewportStatus: p.viewportStatus ?? p.viewport_status ?? null,
        structuredDataPresent: p.structuredDataPresent ?? p.structured_data_present ?? null,
        structuredDataFormat: p.structuredDataFormat ?? p.structured_data_format ?? null,
        structuredDataTypes: p.structuredDataTypes ?? p.structured_data_types ?? null,
        structuredDataPriorityType: p.structuredDataPriorityType ?? p.structured_data_priority_type ?? null,
        // Page Size Measurements
        pageSizeBytes: p.pageSizeBytes ?? p.page_size_bytes ?? null,
        pageSizeStatus: p.pageSizeStatus ?? p.page_size_status ?? null,
        htmlSizeBytes: p.htmlSizeBytes ?? p.html_size_bytes ?? null,
        htmlSizeStatus: p.htmlSizeStatus ?? p.html_size_status ?? null,
        totalResourceSizeBytes: p.totalResourceSizeBytes ?? p.total_resource_size_bytes ?? null,
        resourceSizeBreakdown: p.resourceSizeBreakdown ?? p.resource_size_breakdown ?? null,
        totalWordCount: p.totalWordCount ?? p.total_word_count ?? null,
        visibleWordCount: p.visibleWordCount ?? p.visible_word_count ?? null,
        uniqueWordCount: p.uniqueWordCount ?? p.unique_word_count ?? null,
        textToHtmlRatio: p.textToHtmlRatio ?? p.text_to_html_ratio ?? null,
        sentenceCount: p.sentenceCount ?? p.sentence_count ?? null,
        paragraphCount: p.paragraphCount ?? p.paragraph_count ?? null,
        averageSentenceLength: p.averageSentenceLength ?? p.average_sentence_length ?? null,
        averageParagraphLength: p.averageParagraphLength ?? p.average_paragraph_length ?? null,
        keywordDensity: p.keywordDensity ?? p.keyword_density ?? null,
        // Advanced Word Count Analysis fields
        thinContent: p.thinContent !== null && p.thinContent !== undefined ? Boolean(p.thinContent) : (p.thin_content !== null && p.thin_content !== undefined ? Boolean(p.thin_content) : null),
        thinContentReason: p.thinContentReason ?? p.thin_content_reason ?? null,
        duplicateContent: p.duplicateContent !== null && p.duplicateContent !== undefined ? Boolean(p.duplicateContent) : (p.duplicate_content !== null && p.duplicate_content !== undefined ? Boolean(p.duplicate_content) : null),
        duplicateWithUrls: p.duplicateWithUrls ?? (p.duplicate_with_urls ? (typeof p.duplicate_with_urls === 'string' ? JSON.parse(p.duplicate_with_urls) : p.duplicate_with_urls) : null),
        sectionWordCountMapping: p.sectionWordCountMapping ?? (p.section_word_count_mapping ? (typeof p.section_word_count_mapping === 'string' ? JSON.parse(p.section_word_count_mapping) : p.section_word_count_mapping) : null),
        sectionWordCountBreakdown: p.sectionWordCountBreakdown ?? (p.section_word_count_breakdown ? (typeof p.section_word_count_breakdown === 'string' ? JSON.parse(p.section_word_count_breakdown) : p.section_word_count_breakdown) : null),
        headingWordCountMapping: p.headingWordCountMapping ?? (p.heading_word_count_mapping ? (typeof p.heading_word_count_mapping === 'string' ? JSON.parse(p.heading_word_count_mapping) : p.heading_word_count_mapping) : null),
      })),
      paging: {
        limit,
        offset,
