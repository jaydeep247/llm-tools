...
export class AuditScheduler {
     ...
     */
    async createSchedule(data: Omit<AuditSchedule, 'id' | 'createdAt' | 'totalRuns' | 'successfulRuns' | 'failedRuns'> & { userId?: number }): Promise<number> {
        ...
        const scheduleId = await this.db.insertAuditSchedule({
            name: data.name,
     ...
     */
    async updateSchedule(id: number, updates: Partial<AuditSchedule>): Promise<void> {
        ...
        const dbUpdates = { ...updates } as Partial<import('../database/types.js').AuditSchedule>;
        if (updates.urls) {
            dbUpdates.urls = JSON.stringify(updates.urls);
        }

        await this.db.updateAuditSchedule(id, dbUpdates);
    ...
    async deleteSchedule(id: number): Promise<void> {
        await this.db.deleteAuditSchedule(id);
     ...
     */
    async getAllSchedules(): Promise<AuditSchedule[]> {
        const schedules = await this.db.getAllAuditSchedules();
     ...
     */
    async getSchedule(id: number): Promise<AuditSchedule | null> {
        const schedule = await this.db.getAuditSchedule(id);
     ...
     */
    async getSchedulesToRun(): Promise<AuditSchedule[]> {
        const now = new Date();
        const schedules = await this.db.getEnabledAuditSchedules();
    ...
    private async checkSchedules(): Promise<void> {
        try {
            const schedulesToRun = await this.getSchedulesToRun();
    ...
    private async executeSchedule(schedule: AuditSchedule): Promise<void> {
        ...
        try {
            ...
            executionId = await this.recordExecution(schedule.id);
     ...
     */
    private async recordExecution(scheduleId: number): Promise<number> {
        const executionId = await this.db.insertAuditExecution({
            scheduleId,
    ...
    private async auditUrlWithCache(url: string, device: 'mobile' | 'desktop'): Promise<{ success: boolean }> {
        try {
            ...
            const auditResults = await this.db.getAuditResultsByUrl(url, device, 1);
            const existingResult = auditResults[0];
            if (existingResult) {
            ...
            await saveAudit(result.url, result.device, parsed, result.raw);
     ...
     */
    private async updateExecution(executionId: number, updates: Partial<AuditExecution>): Promise<void> {
        await this.db.updateAuditExecution(executionId, updates);
    }

    /**
     * Get execution history for a schedule
     */
    async getExecutionHistory(scheduleId: number, limit: number = 50): Promise<AuditExecution[]> {
     ...
     * Get all executions
     */
    async getAllExecutions(limit: number = 100): Promise<AuditExecution[]> {
