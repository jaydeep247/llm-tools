    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-(--nd-border) bg-white p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-(--nd-text-primary)">Most Cited Domains Over Time</h3>
          <button
            type="button"
            className="inline-flex items-center gap-1 text-xs text-(--nd-text-secondary) border border-(--nd-border) rounded-md px-2 py-1 bg-(--nd-bg)"
          >
            Over time
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
          <div className="rounded-xl border border-(--nd-border) bg-(--nd-bg) p-3 h-65">
          ...
          <div className="rounded-xl border border-(--nd-border) bg-(--nd-bg) p-3">
            <div className="text-[11px] font-medium uppercase tracking-wider text-(--nd-text-muted) mb-2">
              Domains Shown
            </div>
            <div className="max-h-55 overflow-y-auto space-y-1 pr-1">
              {domainShare.map((domain) => (
                <div
                  key={domain.name}
                  className="flex items-center justify-between gap-2 px-2 py-1.5 rounded hover:bg-white"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    />
                    <span className="text-xs text-(--nd-text-secondary) truncate">{domain.name}</span>
                  </div>
                  <span className="text-xs text-(--nd-text-muted) shrink-0">{domain.percent.toFixed(1)}%</span>
                </div>
              ))}
              {domainShare.length === 0 && !isLoading && !isFetching && (
                <p className="text-xs text-(--nd-text-muted) px-2 py-1.5">No domain data found.</p>
              )}
            </div>
          </div>
        </div>
      </div>
      {(isLoading || isFetching) && <p className="text-xs text-(--nd-text-muted)">Loading sources overview...</p>}
    </div>
  )
}
...
function KpiCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-(--nd-border) bg-(--nd-bg) py-6 text-center">
      <p className="text-[11px] uppercase tracking-wider text-(--nd-text-muted)">{label}</p>
      <p className="text-3xl font-semibold text-(--nd-text-primary) mt-2">{value}</p>
    </div>
  )
}
    ...
    <div className="space-y-1">
      ...
      <div className="relative">
        <select
          ...
          className={cn(
            'w-full h-10 appearance-none rounded-lg border border-(--nd-border) bg-white',
            'px-3 pr-8 text-sm text-(--nd-text-primary) focus:outline-none focus:ring-1 focus:ring-(--nd-purple)'
          )}
        >
          {options.map((option) => (
