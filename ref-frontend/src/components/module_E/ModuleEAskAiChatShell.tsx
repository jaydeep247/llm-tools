...
export type ModuleEAskAiChatTurn = {
  id: string
  role: 'user' | 'assistant'
  content: string
  sources?: string[]
...
type ModuleEAskAiChatShellProps = {
  chatScrollRef: Ref<HTMLDivElement>
  chatMessages: ModuleEAskAiChatTurn[]
  chatInput: string
...
const FALLBACK_SUGGESTIONS = [
  'Summarize my content consistency and entity coverage scores.',
  'Why is my entity coverage low and what should I fix first?',
  'How do my models compare on consistency for tracked prompts?',
]

export function ModuleEAskAiChatShell({
  chatScrollRef,
  chatMessages,
  chatInput,
  setChatInput,
  ...
  useEffect(() => {
    ...
    const assistantIdSet = new Set(assistantMessages.map((m) => m.id))
    ...
    <div className="flex max-h-[min(92vh,880px)] min-h-0 w-full flex-col overflow-hidden rounded-2xl border border-zinc-800/90 bg-gradient-to-b from-[#141416] via-[#111113] to-[#0a0a0b] shadow-2xl shadow-black/60">
      <header className="shrink-0 border-b border-zinc-800/90 bg-zinc-950/40 px-5 py-5 sm:px-8 sm:py-6">
        <div className="flex gap-4">
          <div
            className={cn(
              'flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 to-fuchsia-600 shadow-lg shadow-violet-950/50 ring-2 ring-white/10',
            )}
          >
          ...
          <div className="min-w-0 space-y-1.5">
            ...
            <p className="max-w-2xl text-sm leading-relaxed text-zinc-400 sm:text-base">
              Questions use your latest Module E run: content consistency, entity coverage, model-wise performance,
              and tracked-prompt recommendations.
            </p>
          </div>
        </div>
      </header>
          ...
          <div className="mx-auto max-w-2xl space-y-6 py-4">
            <div className="rounded-2xl border border-dashed border-zinc-700/60 bg-zinc-950/50 p-8 text-center">
              ...
              <p className="text-base leading-relaxed text-zinc-400">
                Start with a suggestion or ask about consistency, coverage, or your tracked prompts.
              </p>
            </div>
            <div>
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:justify-center">
                {chipSuggestions.map((label) => (
                  <button
                    ...
                    className="rounded-xl border border-zinc-700/80 bg-zinc-900/80 px-4 py-3 text-left text-sm text-zinc-300 transition-all hover:border-violet-500/40 hover:bg-zinc-800/80 hover:text-white"
                  >
                    {label}
                  </button>
                ))}
        ...
        {askAIError && chatMessages.length === 0 ? (
          <p className="rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-3 text-base text-red-400">
            {(askAIError as { data?: { message?: string; error?: string } })?.data?.message ||
              ...
              'Something went wrong. Check backend connectivity and try again.'}
          </p>
        ) : null}
      </div>
