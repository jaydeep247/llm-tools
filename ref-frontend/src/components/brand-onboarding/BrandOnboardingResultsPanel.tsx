...
function ScoreChip({ score }: { score: number }) {
  const cls =
    ...
    score > 0   ? 'text-rose-400 bg-rose-500/15 border-rose-500/30' :
                  'text-(--nd-text-muted) bg-(--nd-bg) border-(--nd-border)'
  return (
...
function MentionBadge({ mentioned }: { mentioned: boolean }) {
  return mentioned ? (
    ...
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-medium rounded bg-(--nd-bg)/40 text-(--nd-text-muted) border border-(--nd-border)">
...
function PositionBadge({ position }: { position: string }) {
  const map: Record<string, string> = {
    ...
    not_mentioned: 'text-(--nd-text-muted) bg-(--nd-bg) border-(--nd-border)',
  }
...
function ProviderCard({
  ...
  return (
    <div className={`rounded-lg border ${meta.border} ${meta.bg} p-3`}>
          ...
          {analysis.competitors_mentioned && analysis.competitors_mentioned.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-1">
              {analysis.competitors_mentioned.slice(0, 4).map((c, i) => (
                <span key={i} className="px-1.5 py-0.5 text-[10px] text-(--nd-text-muted) bg-(--nd-bg) border border-(--nd-border) rounded">
                  {c.name}
                </span>
              ))}
      ...
      {response && (
        <div className="mt-2 pt-2 border-t border-(--nd-border)">
          <button
            onClick={() => setShowResponse(!showResponse)}
            className="text-[10px] text-(--nd-text-muted) hover:text-(--nd-text-secondary) underline underline-offset-2"
          >
            {showResponse ? 'Hide response' : 'View LLM response'}
          </button>
          {showResponse && (
            <div className="mt-1.5 p-2 bg-white rounded-lg border border-(--nd-border) max-h-36 overflow-y-auto">
              <p className="text-[10px] text-(--nd-text-muted) leading-relaxed whitespace-pre-wrap">{response}</p>
            </div>
          )}
        </div>
      )}
...
function PromptRow({ result, index }: { result: PromptResult; index: number }) {
  ...
  return (
    <div className="border border-(--nd-border) rounded-xl bg-(--nd-bg) overflow-hidden">
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full flex items-start gap-3 p-3.5 text-left hover:bg-(--nd-bg) transition-colors"
      >
        <span className="text-[10px] font-mono text-zinc-600 mt-0.5 shrink-0">
          {String(index + 1).padStart(2, '0')}
        </span>
        <div className="flex-1 min-w-0">
          <p className="text-[13px] text-(--nd-text-secondary) leading-relaxed line-clamp-2">{result.prompt}</p>
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] text-(--nd-text-muted) bg-(--nd-bg) border border-(--nd-border) rounded-full">
      ...
      {expanded && (
        <div className="border-t border-(--nd-border) px-3.5 pb-3.5">
...
function ProviderSummaryCard({
  ...
  return (
    <div className={`rounded-xl border ${meta.border} ${meta.bg} p-4`}>
      ...
      <div className="text-[10px] text-(--nd-text-muted) mt-0.5">avg visibility score</div>
      <div className="mt-3 space-y-1.5 text-[11px] text-(--nd-text-muted)">
        <div className="flex items-center justify-between">
          <span>Mention rate</span>
          <span className="text-(--nd-text-secondary) font-medium">{mentionRate}%</span>
        </div>
        <div className="flex items-center justify-between">
          <span>Competitors seen</span>
          <span className="text-(--nd-text-secondary) font-medium">{competitorCount}</span>
        </div>
      </div>
    </div>
  )
...
export function BrandOnboardingResultsPanel({ jobId }: BrandOnboardingResultsPanelProps) {
  ...
  if (!jobId) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <div className="w-12 h-12 rounded-2xl bg-(--nd-bg) flex items-center justify-center mb-4">
          <BarChart3 className="h-5 w-5 text-(--nd-text-muted)" />
        </div>
        <p className="text-sm text-(--nd-text-muted)">No session job found.</p>
      </div>
    )
  }
    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="flex flex-col items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-(--nd-bg) flex items-center justify-center">
            <Loader2 className="h-5 w-5 animate-spin text-(--nd-text-muted)" />
          </div>
          <span className="text-sm text-(--nd-text-muted)">Loading brand survey…</span>
        </div>
      </div>
    )
  }
  ...
  if (results.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <div className="w-12 h-12 rounded-2xl bg-(--nd-bg) flex items-center justify-center mb-4">
          <Sparkles className="h-5 w-5 text-(--nd-text-muted)" />
        </div>
        <p className="text-base font-semibold text-(--nd-text-secondary) mb-1">No Brand Survey Results Yet</p>
  ...
  return (
    <div className="space-y-6 py-2">
      {/* Header */}
      <div>
        <h2 className="text-lg sm:text-xl font-bold text-(--nd-text-primary)">AI Brand Survey</h2>
        <p className="text-xs sm:text-sm text-(--nd-text-muted) mt-1">
      ...
      {topics.length > 0 && (
        <div>
          <p className="text-[11px] text-(--nd-text-muted) uppercase tracking-widest mb-2">Topics Tracked</p>
          <div className="flex flex-wrap gap-1.5">
            {topics.map((t, i) => (
              <span key={i} className="px-2.5 py-0.5 text-[11px] text-(--nd-text-secondary) bg-(--nd-bg) border border-(--nd-border) rounded-full">
                {t}
              </span>
            ))}
          </div>
      ...
      {/* Prompt results list */}
      <div>
        <p className="text-[11px] text-(--nd-text-muted) uppercase tracking-widest mb-3">
