...
export function AlertBanner({ alerts, onNavigate }: AlertBannerProps) {
  ...
  return (
    <div className="rounded-xl border border-red-500/40 bg-red-950/30 px-4 py-3 flex items-start gap-3 animate-fade-in">
      <AlertCircle className="h-5 w-5 text-red-400 shrink-0 mt-0.5" />
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-red-200 leading-snug">{top.message}</p>
        {top.recommendation && (
          <p className="text-xs text-red-300/70 mt-0.5 leading-relaxed">{top.recommendation}</p>
        )}
        {criticals.length > 1 && (
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button
          ...
          className="flex items-center gap-1 text-xs font-semibold bg-red-500 hover:bg-red-400 text-white px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
        >
          Fix Now
        ...
        <button
          ...
          className="p-1 text-red-400/60 hover:text-red-200 rounded transition-colors cursor-pointer"
