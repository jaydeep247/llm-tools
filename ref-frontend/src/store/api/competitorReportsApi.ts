...
export const competitorReportsApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getCompetitorReport: builder.query<CompetitorReportsResponse, { projectId: string; period: CRPeriod }>({
      query: ({ projectId, period }) =>
        `/reports/competitors?project_id=${encodeURIComponent(projectId)}&period=${period}`,
      transformResponse: (response: { data: CompetitorReportsResponse }) => response.data,
    }),
  }),
})

export const { useGetCompetitorReportQuery } = competitorReportsApi
